...
class AppTests(unittest.TestCase):
    ...
    def test_maintenance_status_and_backup_preview(self):
        ...
        self.assertEqual(status.get_json()["current"], "0.9.0-rc.5")
        ...
        self.assertEqual(summary["application_version"], "0.9.0-rc.5")
    ...
    def test_health_reports_running_version_and_agent_api(self):
        ...
        self.assertEqual(health["version"], "0.9.0-rc.5")
    ...
    def test_windows_agent_download(self):
        ...
        original_probe = self.module._remote_agent_installer_available
        try:
            self.module._remote_agent_installer_available = lambda _url: True
            installer = self.client.get("/download/windows-agent.exe")
            self.assertEqual(installer.status_code, 302)
            self.assertEqual(
                installer.headers["Location"],
                "https://github.com/HypeTek/hypetek-gamevault/releases/download/v0.9.0-rc.5/HypeTek-Mission-Control-Agent-Setup.exe",
            )
            ...
            self.assertIn("/download/windows-agent.zip", fallback.headers["Location"])
        finally:
            self.module._remote_agent_installer_available = original_probe
    ...
    def test_appearance_settings_and_scan_exclusions(self):
        ...
        self.assertIn("/static/app.js?v=0.9.0-rc.5", html)
        self.assertIn("/static/i18n.js?v=0.9.0-rc.5", html)
        self.assertIn("/static/app.css?v=0.9.0-rc.5", html)
        ...
        self.assertEqual(settings["version"], "0.9.0-rc.5")
    ...
    def test_new_ltr_language_login_copies_are_available(self):
        expected = {
            "zh": "管理员密码",
            "tlh": "loHwI&#39; pegh mu&#39;",
            "sjn": "Tharan i hervenn",
        }
        for language, label in expected.items():
            self.module.settings_store.update({"ui_language": language})
            page = self.client.get("/login").get_data(as_text=True)
            self.assertIn(f'lang="{language}" dir="ltr"', page)
    ...
    def test_translator_status_uses_stored_server_side_credentials(self):
        ...
        try:
            ...
            self.module.validate_translator = original_validate
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_json(), {"configured": True, "reachable": True, "languages": ["de", "en", "ru", "tlh", "sjn"], "native_languages": ["de", "en", "ru"]})
    ...
    def test_translator_test_accepts_unsaved_address_without_persisting_it(self):
        ...
        try:
            ...
            self.module.validate_translator = original_validate
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_json(), {"configured": True, "reachable": True, "languages": ["de", "en", "tlh", "sjn"], "native_languages": ["de", "en"]})
