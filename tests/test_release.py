...
class ReleaseMetadataTests(unittest.TestCase):
    def test_release_versions_are_consistent(self):
        ...
        self.assertIn('data-close-settings class="secondary" data-i18n="common.cancel"', template)
        ...
        self.assertIn('id="editCoverFit"', template)
        self.assertIn('id="editCoverZoom"', template)
        self.assertIn('id="resetCoverFraming"', template)
        self.assertIn('cover_fit: document.querySelector("#editCoverFit").value', javascript)
        self.assertIn('cover_zoom: Number(document.querySelector("#editCoverZoom").value)', javascript)
        ...
        self.assertIn('Object.assign(UI_PACKS.tlh, {', translations)
        self.assertIn('Object.assign(UI_PACKS.sjn, {', translations)
        self.assertIn('"settings.uiLanguageHint":"Holmey Qap;', translations)
        self.assertIn('"settings.uiLanguageHint":"Lambath nestannen;', translations)
        self.assertIn('"settings.removeRawg":"RAWG ngaQHa\'moHwI\' teq"', translations)
        self.assertIn('"settings.removeRawg":"Drego annon RAWG"', translations)
        self.assertIn('const COVER_EDITOR_PACKS = {', translations)
        for language in ("de", "en", "ru", "it", "fr", "es", "pt", "pl", "nl", "tr", "ar", "zh", "tlh", "sjn"):
            self.assertIn(f'  {language}: {{"editor.coverFit"', translations)
        ...
        self.assertIn('html[dir="rtl"] body[data-style="lcars"] main{border-left-width:0;border-right:var(--lcars-rail-width) solid transparent', stylesheet)
        self.assertIn('html[dir="rtl"] body[data-style="lcars"] .agent-note-content{padding-left:0;padding-right:calc(var(--lcars-rail-width) + 24px)', stylesheet)
        self.assertIn('html[dir="rtl"] body[data-style="lcars"] dialog{border-left:0;border-right:24px solid var(--orange);border-radius:5px 38px 5px 38px}', stylesheet)
        ...
        self.assertIn('function Convert-MappedDrivePathToUnc', agent)
        self.assertIn('Get-SmbMapping -LocalPath $drive', agent)
        self.assertIn('New-Object -ComObject WScript.Network', agent)
        self.assertIn('function Start-MissionControlInstaller', agent)
        self.assertIn('$startInfo.UseShellExecute = $true', agent)
        self.assertIn('Start-MissionControlInstaller $source', agent)
        self.assertNotIn('Start-Process -FilePath $source', agent)
        ...
        self.assertIn("version: 1.0.4", app_yaml)
