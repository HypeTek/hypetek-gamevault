...
def update_game(game_id: str):
    ...
    if "cover_fit" in payload:
        payload["cover_fit"] = str(payload["cover_fit"] or "cover").strip().lower()
        if payload["cover_fit"] not in {"cover", "contain"}:
            return jsonify(error="Ungültige Coverdarstellung"), 400
    if "cover_zoom" in payload:
        try:
            payload["cover_zoom"] = max(100, min(200, int(payload["cover_zoom"])))
        except (TypeError, ValueError):
            return jsonify(error="Ungültiger Coverzoom"), 400
