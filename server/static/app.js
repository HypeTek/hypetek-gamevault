...
function renderPagination(total, pageCount, firstIndex, shown) {
  ...
  const firstGlyph = rtl ? "»" : "«";
  const previousGlyph = rtl ? "→" : "←";
  const nextGlyph = rtl ? "←" : "→";
  const lastGlyph = rtl ? "«" : "»";
  pagination.innerHTML = `<span>${start}–${end} / ${total}</span><div class="pagination-controls"><button type="button" class="secondary pagination-edge" data-page="first" aria-label="${escapeHtml(tr("pagination.first"))}" title="${escapeHtml(tr("pagination.first"))}" ${state.page <= 1 ? "disabled" : ""}>${firstGlyph}</button><button type="button" class="secondary" data-page="previous" aria-keyshortcuts="Alt+ArrowLeft" ${state.page <= 1 ? "disabled" : ""}>${previousGlyph} ${tr("pagination.previous")}</button><span class="pagination-pages">${pageItems}</span><strong>${tr("pagination.page", {current: state.page, total: pageCount})}</strong><button type="button" class="secondary" data-page="next" aria-keyshortcuts="Alt+ArrowRight" ${state.page >= pageCount ? "disabled" : ""}>${tr("pagination.next")} ${nextGlyph}</button><button type="button" class="secondary pagination-edge" data-page="last" aria-label="${escapeHtml(tr("pagination.last"))}" title="${escapeHtml(tr("pagination.last"))}" ${state.page >= pageCount ? "disabled" : ""}>${lastGlyph}</button></div>`;
