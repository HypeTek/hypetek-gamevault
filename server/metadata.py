...
def _get_json(url: str, timeout: int = 12) -> dict:
    request = Request(url, headers={"Accept": "application/json", "User-Agent": "HypeTek-Mission-Control/0.9.0-rc.4"})
    try:
...
def fetch_thegamesdb_image(image_url: str) -> tuple[bytes, str]:
    ...
    request = Request(image_url, headers={"Accept": "image/*", "User-Agent": "HypeTek-Mission-Control/0.9.0-rc.4"})
    try:
